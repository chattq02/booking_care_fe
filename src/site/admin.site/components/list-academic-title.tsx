...
import academicTitleAdmin from "../apis/academic-tilte";
...
export default function ListAcademicTitle({
  defaultValue,
  onChange,
  style,
  mode,
}: IProps) {
  const [data, setData] = useState<ResponseAcademicTitle[]>([]);
  ...
  const fetchAPI = useCallback(
    async (pageNum = 1, search = "", append = false) => {
      try {
        setIsLoading(true);
        const res = await academicTitleAdmin.getList({
          keyword: search,
          page: pageNum,
          per_page: 10,
        });
